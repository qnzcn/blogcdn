...
if (isset($post_id) && empty($ref)):
    ...
    if ($PostPay->isPayPost() || $cao_is_post_free) {
        ...
        // 判断会员类型 判断下载次数
        $vip_status = $CaoUser->vip_status();        $this_vip_downum = $CaoUser->cao_vip_downum($uid,$vip_status);
        ...
        if ($this_vip_downum['is_down'] || $PostPay->isPayPost() ) {
            if (_cao('is_all_down_num','0') && !$this_vip_downum['is_down']) {
                wp_die('今日下载次数已用：'.$this_vip_downum['today_down_num'].'次,剩余下载次数：'.$this_vip_downum['over_down_num'],'下载次数超出限制,如果有已经购买无法下载的，请等待明天下载');exit();
            }
            $is_add_down_log = false;
            //没有真实购买 但是使用免费权限下载 将计算下载次数
            if (!$PostPay->isPayPost() && $cao_is_post_free) {                update_user_meta($uid, 'cao_vip_downum', $this_vip_downum['today_down_num'] + 1); //更新+1		
                $is_add_down_log = $PostPay->add_down_log();                // 更新完成 更新资源销售数量 输出成功信息
                $before_paynum = get_post_meta($post_id, 'cao_paynum', true);                update_post_meta($post_id, 'cao_paynum', (int) $before_paynum + 1);
            }
            if (_cao('is_all_down_num','0') && !$is_add_down_log) {                $PostPay->add_down_log();            }
            # // 开始下载缓冲...
            $flush = _download_file($_downurl);            exit();
        } else {
