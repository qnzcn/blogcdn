...
function cao_entry_header($options = array()) {
    ...
    if ($options['author'] || $options['category'] || $options['date'] || $options['comment'] || $options['like']): 
        ...
        if ($options['author']){
            ...
            echo '<span class="meta-author"><a href="'.esc_url( get_author_posts_url($author_id,get_the_author_meta( 'display_name', $author_id ) )).'">'.get_avatar($author_id).get_the_author_meta( 'display_name', $author_id ).'</a></span>';
        }
        ...
        if ($options['category']) {
            ...
            if ($categories) {
                ...
                foreach ($categories as $key => $category) {
                    if ($key == 3) {break;}
                    echo '<a'._target_blank().' href="'.esc_url(get_category_link($category->term_id)).'" rel="category"><i class="dot"></i>'.esc_html($category->name).'</a>';
                }
                echo '</span>';
            }
        }
        ...
        if ($options['date']) {
            ...
            echo '<a'._target_blank().' href="'.esc_url(get_the_permalink($post_id)).'"><time datetime="'.get_the_date('c', $post_id).'">'._get_post_time().'</time></a>';
            echo '</span>';
        }
        ...
        if ($options['comment'] && !post_password_required($post_id) && (comments_open($post_id) || get_comments_number($post_id))) {
            ...
            echo '<a'._target_blank().' href="'.esc_url(get_the_permalink($post_id) . '#comments').'"><i class="fa fa-comments-o"></i> '.get_comments_number($post_id).'</a>';
            echo '</span>';
        }
