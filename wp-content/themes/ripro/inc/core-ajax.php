...
function user_register()
{
    ...
    if ($user_pass != $user_pass2) {
        echo json_encode(array('status' => '0', 'msg' => '两次输入的密码不一致'));exit;
    }
    // 是否需要邮箱验证
...
function charge_pay()
{
    ...
    if ($charge_num < $min_cahrge_num) {
        echo json_encode(array('status' => '0', 'msg' => '最低充值数量限额：'.$min_cahrge_num));exit;
    }
    if ($charge_num > $max_cahrge_num) {
        echo json_encode(array('status' => '0', 'msg' => '最高充值数量限额：'.$max_cahrge_num));exit;
    }
    ...
    if ($pay_type == 2) {
        ...
        if (wp_is_mobile() && $wxPayConfig['is_mobile'] && !is_weixin_view()) {
            ...
            if($pay->checkResult())
            {
                ...
                echo json_encode(array('status' => '1', 'type' => '3', 'msg' => $jsapiParams, 'img' => '', 'num' => $order_trade_no));exit;
            }else{
                echo json_encode(array('status' => '0', 'msg' => '未绑定公众号登录或网络错误'));exit;
            }
    ...
    if ($pay_type == 5) {
        ...
        try {
            ...
            if ($XHpayConfig['is_pop_qrcode'] && !is_weixin_view()) {
                ...
                echo json_encode(array('status' => '1', 'type' => '2', 'rurl' => $result['url'], 'qrcode' => '', 'msg' => $order_trade_no));exit;
            }
...
function go_post_pay()
{
    ...
    if (get_post_meta($post_id, 'cao_close_novip_pay', true) && !$CaoUser->vip_status()) {
        echo json_encode(array('status' => '0', 'msg' => '该资源为会员专属资源，普通用户无购买权限！'));exit;
    }
...
function add_pay_post()
{
    ...
    if (get_post_meta($post_id, 'cao_close_novip_pay', true) && !$CaoUser->vip_status()) {
        echo json_encode(array('status' => '0', 'msg' => '该资源为会员专属资源，普通用户无购买权限。开通会员后即可正常购买获取。'));exit;
    }
