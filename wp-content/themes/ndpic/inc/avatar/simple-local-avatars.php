...
class Simple_Local_Avatars {
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		...
		wp_enqueue_script( 'simple-local-avatars', get_template_directory_uri( '', __FILE__ ) . '/inc/avatar/simple-local-avatars' . $script_name_append . '.js', array('jquery'), false, true );
...
<style>
	...
	<table class="form-table">
		<tr>
			<th scope="row"><label for="simple-local-avatar"><?php _e('上传头像','simple-local-avatars'); ?></label></th>
			...
			?>
					...
					<p style="display: inline-block; width: 26em;">
						...
						<?php } ?>
					<p>
						<?php if ( current_user_can( 'upload_files' ) && did_action( 'wp_enqueue_media' ) ) : ?><a href="#" class="button hide-if-no-js" id="simple-local-avatar-media"><?php _e( '选择/上传', 'simple-local-avatars' ); ?></a> &nbsp;<?php endif; ?>
						<a href="<?php echo $remove_url; ?>" class="button item-delete submitdelete deletion" id="simple-local-avatar-remove"<?php if ( empty( $profileuser->simple_local_avatar ) ) echo ' style="display:none;"'; ?>><?php _e('删除','simple-local-avatars'); ?></a>
					</p>
			<?php
				} else {
			...
			<td colspan="2">
				<fieldset id="simple-local-avatar-ratings" <?php disabled( empty( $profileuser->simple_local_avatar ) ); ?>>
					...
					<?php
						...
						endforeach;
					?>
					<p class="description"><?php _e( '如果本地头像不适用于此站点，将尝试使用Gravatar头像。', 'simple-local-avatars' ); ?></p>
				</fieldset></td>
		</tr>
	</table>
	<?php
