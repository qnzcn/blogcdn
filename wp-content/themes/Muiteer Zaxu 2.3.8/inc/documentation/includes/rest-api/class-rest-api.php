...
class MuiteerDocs_REST_API extends WP_REST_Controller {
    ...
    public function handle_feedback($request) {
        ...
        muiteerdocs_doc_feedback_email( $id, $name, $email, $request['subject'], $request['message'] );
    ...
    public function search_docs($request) {
        ...
        if ( $request['in'] ) {
            ...
            $children_docs = muiteerdocs_get_posts_children($request['in'], 'docs');
        ...
        if ($page > $max_pages && $total_posts > 0) {
            return new WP_Error('rest_docs_invalid_page_number', __( 'The page number requested is larger than the number of pages available.', 'muiteer'), array('status' => 400) );
        }
        $response = rest_ensure_response($result);
    ...
    public function create_item_permissions_check($request) {
        ...
        if ( empty( $request['name'] ) ) {
            return new \WP_Error( 'rest_doc_invalid_name', __('No name given', 'muiteer') );
        }
        if ( empty( $request['email'] ) ) {
            return new \WP_Error( 'rest_doc_invalid_email', __('No email given', 'muiteer') );
        }
        return true;
    }
