...
// @codekit-prepend "src/media-insert.js";
...
(function($) {
  ...
  let app = new Vue({
    ...
    mounted: function() {
      ...
      // Get gallery info from post_content_filtered input.
      let gallery = $('#woowgallery-data').val(),
        new_gallery;
      this.gallery = gallery ? JSON.parse(gallery) : [];

      // Open custom media modal on button click
      $('#woowgallery').on('click', '.woowgallery-add-wpmedia', (e) => {
        e.preventDefault();
        woowgallery.actions.MediaFramePost([], {});
      });
    ...
    methods: {
      // Move selected items to another gallery
