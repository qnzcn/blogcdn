...
function pgc_sgb_render_albums_blocks_callback($atr, $content)
{
	...
	if (array_key_exists('useGlobalSettings', $atr)) {
		$useGlobalSettings = $atr['useGlobalSettings'];
		if (isset($useGlobalSettings) && $useGlobalSettings) {
			$skinGlobalOptions = $pgc_sgb_skins_presets['pgc_sgb_' . $skinType];
	...
	$noscript = '<noscript><div class="simply-gallery-amp pgc-sgb-album ' . esc_attr($align) . '"><div class="sgb-gallery">'
		. pgc_sgb_noscript_covers($atr['galleries'])
		. '</div></div></noscript>';
	$preloaderColor = isset($atr['preloaderColor']) ? $atr['galleryPreloaderColor'] : '#d4d4d4';
...
function pgc_sgb_register_posts_block()
{
	...
	wp_register_script(
		...
		array('wp-blocks', 'wp-i18n', 'wp-element', 'wp-editor', 'wplink', 'wp-data', 'media', 'media-grid', 'backbone', 'code-editor', 'csslint', PGC_SGB_SLUG . '-script'),
		PGC_SGB_VERSION,
		false
	);
	$globalJS = array(
